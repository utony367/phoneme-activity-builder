...
<body>
  ...
  <script>
    ...
    function render() {
      ...
      for (let rowIndex = 0; rowIndex < activity.maxAttempts; rowIndex += 1) {
        ...
        for (let column = 0; column < activity.target.text.length; column += 1) {
          ...
          const stored = guesses[rowIndex];
          if (stored) {
            input.value = stored.letters[column].toUpperCase();
            input.classList.add(stored.result[column]);
          }
          input.disabled = finished || Boolean(stored) || rowIndex !== currentRow;
    ...
    document.getElementById("reset").addEventListener("click", () => { currentRow = 0; finished = false; guesses = []; message.textContent = ""; render(); });
    render();
  </script>
</body>
</html>`;
