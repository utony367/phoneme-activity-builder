...
The application is Dockerised with a Node 22 Alpine multi-stage Dockerfile. Docker describes multi-stage builds as a way to structure an image around the artefacts required to run an application (Docker, Inc., n.d.). At container startup, an entrypoint applies Prisma migrations before starting the standalone Next.js server. The SQLite file is stored at `/data/app.db`; mounting a named volume at `/data` means saved records survive a new container. This makes the project reproducible on another computer with Docker Desktop.
