...
Automated tests cover validation, normalisation, health checking, client API error handling, and both generators. GitHub Actions runs a production dependency audit gate, Prisma generation and schema validation, linting, unit tests, a production build, Docker build, and a container smoke test. The audit run on 14 September 2026 found no production vulnerabilities. The full development tree retained one high transitive `js-yaml` advisory and two moderate Vitest-related advisories; these are development-only, and the available Vitest upgrade is major. The smoke test checks both `/health` and the activity collection endpoint. `GET /health` returns `{"status":"ok"}` with HTTP 200.
